flowchart TD
    ...
    C -->|Tiếp tục| D[Lấy mẫu ngẫu nhiên tham số vật thể<br/>o_x ~ U_0.5_4.5, o_y ~ U_0.5_4.5<br/>r ~ U_0.15_0.40, h ~ U_1.2_1.9]
    ...
    G -->|Tiếp tục| H[Tính hình học truyền<br/>d_vec = r_j - l_i, d = norm_d_vec<br/>cos_phi = -d_z/d, cos_psi = -d_z/d]
    ...
    R --> S[Thêm nhiễu Gaussian vào P_map_i<br/>P_noisy = max_P_map_i + N_0_sigma2, 1e-12]
    
    S --> T[Biến đổi log sang dB<br/>RSS_i = 10 * log10_P_noisy]
    
    T --> U[Ghép đặc trưng<br/>X_k = concat_RSS_1_RSS_2_RSS_3_RSS_4]
